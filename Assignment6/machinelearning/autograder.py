...
class Tracker(object):
    def __init__(self, questions, maxes, prereqs, mute_output):
        ...
        import time
        print('\nStart at %d:%02d:%02d' % time.localtime()[3:6])
...
""".format(prereq, q, q, prereq))
    ...
    def finalize(self):
        ...
        if sum(self.points.values()) == sum([self.maxes[q] for q in self.questions]) and \
           time.time() - self.start_time > 600 and \
...
run 'python submission_autograder.py' and submit 'machinelearning.tar' to
...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        ...
        for point in (random_point, np.zeros_like(random_point)):
            ...
            expected_prediction = np.where(np.dot(point, p.get_weights().data.T) >= 0, 1, -1).item(0)
