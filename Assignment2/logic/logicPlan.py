...
def sentence1() -> Expr:
    ...
    clause1 = disjoin(A,B)
    clause2 = nA % disjoin(nB,C)
    clause3 = disjoin(nA,nB,C)
...
def sentence2() -> Expr:
    ...
    clause1 = C % disjoin(B, D)
    clause2 = A >> conjoin(nB, nD)
    clause3 = ~ conjoin(B, nC) >> A
    clause4 = nD >> C
...
def sentence3() -> Expr:
    ...
    clause1 = Alive_1 % disjoin(conjoin(Alive_0, ~Killed_0), conjoin(~Alive_0, Born_0))
    clause2 = ~ conjoin(Alive_0, Born_0)
    clause3 = Born_0
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([disjoin([ ~x for x in i]) for i in itertools.combinations(literals, 2)])
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(pacman_str, x_cor, y_cor, time = t) for x_cor, y_cor in non_outer_wall_coords]))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB0.append(PropSymbolExpr(pacman_str, x1, y1, time = 1))
    KB1.append(~PropSymbolExpr(pacman_str, x1, y1, time = 1))

    model0 = findModel(conjoin(KB0))
    model1 = findModel(conjoin(KB1))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            gowall.append(PropSymbolExpr(pacman_str, x, y, time = t))
        KB.append(exactlyOne(gowall))
        ...
        for direction in actions:
            gostep.append(PropSymbolExpr(direction, time = t))
        KB.append(exactlyOne(gostep))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        food_check = []
        for x, y in food:
            food_check.append(~PropSymbolExpr(food_str, x, y, time = t))
        model = findModel(conjoin(food_check) & conjoin(KB))
        if model:
            return extractActionSequence(model, actions)

        # food update
        for x, y in food:
            if_pacman_current = PropSymbolExpr(pacman_str, x, y, time = t)
            if_food_current = PropSymbolExpr(food_str, x, y, time = t)
            no_food_future = ~PropSymbolExpr(food_str, x, y, time = t + 1)
            no_food_current = ~PropSymbolExpr(food_str, x, y, time = t)
            KB.append(disjoin(no_food_current, conjoin(if_pacman_current, if_food_current) % no_food_future))
