...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        if jailPosition == ghostPosition:
            ...
            return 0
        if noisyDistance is None:
            # print(jailPosition, ghostPosition, pacmanPosition)
            return 0
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        tmp_belief = DiscreteDistribution()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for element in newPosDist.keys():
                tmp_belief[element] += self.beliefs[oldPos] * newPosDist[element]
        tmp_belief.normalize()
        self.beliefs = tmp_belief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles - self.numParticles // len(self.legalPositions) * len(self.legalPositions)):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for par in self.particles:
            dist[par] += self.getObservationProb(observation, pacmanPosition, par, jailPosition)
        dist.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            tmp_particle.append(sample)
        self.particles = tmp_particle
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for par in self.particles:
            ...
            for i in range(self.numGhosts):
                prod *= self.getObservationProb(observation[i], pacmanPosition, par[i], self.getJailPosition(i))
            dist[par] += prod
        dist.normalize()
