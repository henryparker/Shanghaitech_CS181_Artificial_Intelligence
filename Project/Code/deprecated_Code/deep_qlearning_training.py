    ...
    for epoch in range(100):
        with torch.no_grad():
            ...
            for _ in trange(5400): #5000
                qvalue = target_model(features[-1],oldAction).reshape(1,4)
                # epsilon-greedy
                if random.random() < 1-epoch/100:
        ...
        if epoch%10==0:
            torch.save(model.state_dict(), 'checkpoints/deep_qlearning_model_'+str(epoch)+'.pth')
            state_dict=torch.load('checkpoints/deep_qlearning_model_'+str(epoch)+'.pth')
