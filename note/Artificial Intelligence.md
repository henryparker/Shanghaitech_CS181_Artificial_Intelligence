...
2. **向后删除**：从根节点后第一个子节点开始，如果父节点存在弧不相容的分配，将该值删去

3. **向前分配**：从最后一个结点开始，选择任意不和子节点冲突的剩余值

**特征**：

1. 向后删除以后，所有的根指向叶的弧都是相容的
2. 如果所有根指向叶的弧是相容的，那么向前分配就不会回溯
...
- 蕴含：如果$S1$和$S2$是命题，$S1\Rightarrow S2$是命题
...
2. 重复以下的步骤：
   1. 随机选一个非证据变量$X$
      1. 根据$P(X|markov\_blanket(X))$，重新赋值$X$
